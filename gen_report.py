...
data = [
    ...
    ['C端前端', '价格展示', '已完成', '可配置', '管理后台设置', '价格自由填写'],
    ['C端前端', '价格校验', '待开发', '缺失', '需加校验', '建议限制最低0.01、最高99999'],
    ['C端前端', '登录/注册', '已完成', '可配置', '短信服务商配置', '验证码发送'],
...
for col, header in enumerate(['配置项', '配置位置', '配置方式', '当前状态'], 1):
...
configs = [
    ['技能名称/价格/上下架/分类', '管理后台 → 技能管理', '编辑页面输入', '可用'],
    ['收货地址', 'C端个人中心', '用户自助操作', '可用'],
    ['优惠券发放', '管理后台 → 优惠券', '选择用户发放', '可用'],
    ['用户封禁/解封', '管理后台 → 用户管理', '点击操作按钮', '可用'],
    ['余额调整', '管理后台 → 用户管理', '输入增减值', '可用'],
    ['订单查看/退款', '管理后台 → 订单管理', '列表筛选/处理', '可用'],
    ['分类增删改查', '管理后台 → 分类管理', '后台操作', '可用'],
    ['数据库/Redis/Nacos配置', '后端 application.yml', '修改配置文件', '可用'],
    ['管理员账号', '管理后台源码', '需改代码', '需改代码'],
    ['免费/付费区分', '需加字段', '待开发', '待开发'],
]
for row_idx, row_data in enumerate(configs, 4):
    for col_idx, value in enumerate(row_data, 1):
        cell = ws2.cell(row=row_idx, column=col_idx, value=value)
        cell.border = thin_border
        if col_idx == 4:
            cell.fill = done_fill if '可用' in value else config_fill
...
ws3['A1'] = 'COCO CLAW 写死项汇总'
ws3['A1'].font = Font(bold=True, size=16)
ws3.merge_cells('A1:D1')
for col, header in enumerate(['位置', '功能点', '当前状态', '建议'], 1):
...
hardcodes = [
    ['C端首页', 'Banner轮播图', '写死3张图', '建议开发后台Banner管理'],
    ['C端首页', '热门推荐位', '写死推荐商品', '建议开发手动推荐功能'],
    ['C端前端', '价格货币符号', '固定显示¥', '建议后台可配置'],
    ['C端前端', '页面布局结构', '固定布局', '如需改需重写页面'],
    ['管理后台', '数据统计指标', '固定4个指标', '建议允许自定义'],
    ['管理后台', '管理员账号', '代码中写死', '建议改成数据库存储'],
    ['后端', '分页大小', '固定30条/页', '建议后台可配置'],
    ['后端', 'Token有效期', '固定7天', '建议后台可配置'],
]
for row_idx, row_data in enumerate(hardcodes, 4):
    for col_idx, value in enumerate(row_data, 1):
...
for col, header in enumerate(['功能', '所属模块', '优先级', '负责人', '说明'], 1):
...
todos = [
    ...
    ['价格校验', '后端+前端', '中', '可找我开发', '限制0.01~99999'],
    ['Banner管理', '管理后台+C端', '中', '可找我开发', '后台上传Banner图'],
    ['热门推荐后台配置', '管理后台+C端', '中', '可找我开发', '后台手动设置推荐'],
    ['管理员账号后台管理', '管理后台', '中', '可找我开发', '从数据库读取'],
    ['短信接口对接', '后端+前端', '中', '同事', '验证码发送'],
    ['用户等级系统', '全栈', '低', '可找我开发', 'VIP/普通用户区分'],
]
for row_idx, row_data in enumerate(todos, 4):
    for col_idx, value in enumerate(row_data, 1):
...
wb.save('C:/Users/ROG/CodeBuddy/20260404122544/COCO_CLAW_系统配置说明.xlsx')
print('Done')
